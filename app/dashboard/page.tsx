...
export default async function DashboardPage() {
  let rawData = await getLiftData();
  const data = Array.isArray(rawData) ? rawData : [];

  // Calculations for Summary
  const totalLifts = data.reduce((acc, curr) => acc + parseInt(curr.Total || '0'), 0);
  const uniqueContents = new Set(data.map(item => item.libraryItemLabel)).size;
  const uniqueStores = new Set(data.map(item => item.storeLocation)).size;
  const uniqueScreens = new Set(data.map(item => item.screenId)).size;
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <aside className="fixed left-0 top-0 hidden h-full w-64 border-r border-slate-200 bg-white p-6 lg:block">
        <div className="mb-10 flex items-center gap-2">
          ...
          <span className="text-xl font-bold tracking-tight text-slate-800">OMG Interactive</span>
        </div>
        ...
        <nav className="space-y-1">
          ...
          <Link href="/dashboard/analytics" className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900">
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            </svg>
            Analytics (NEW)
          </Link>
          <a href="#" className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-400 cursor-not-allowed">
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <main className="lg:pl-64">
        <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/80 backdrop-blur-md">
          <div className="flex items-center justify-between px-8 py-4">
            <div>
              <h2 className="text-2xl font-bold text-slate-800">Lift and Learn Insights</h2>
              <p className="text-sm text-slate-500">Real-time engagement metrics across stores</p>
            </div>
        ...
        <div className="p-8">
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 bg-slate-50/50 px-6 py-4 flex items-center justify-between">
              <h3 className="font-bold text-slate-800 text-lg">Detailed Activity Log</h3>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Content Label</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Store / Section</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Device</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Total Interaction</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Unique ID</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {data.map((item, idx) => (
                    <tr key={idx} className="transition-colors hover:bg-blue-50/30 group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-slate-800">{item.libraryItemLabel}</div>
                            <div className="text-xs text-slate-400">ID: {item.libraryItemId}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          >
                            <div className="text-sm font-bold text-slate-700 group-hover/link:text-blue-600 transition-colors flex items-center gap-1">
                              {item.storeLocation}
                              <svg className="h-3 w-3 opacity-0 -translate-x-1 group-hover/link:opacity-100 group-hover/link:translate-x-0 transition-all text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                           <div className="text-lg font-bold text-blue-700">{item.Total}</div>
                           <div className="h-2 w-24 rounded-full bg-slate-100 overflow-hidden">
                              <div 
                                className="h-full bg-blue-500 rounded-full" 
                                style={{ width: `${Math.min((parseInt(item.Total)/50)*100, 100)}%` }}
                              ></div>
                           </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <code className="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-500">#{item.itemId}</code>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {data.length === 0 && (
              <div className="py-20 text-center">
                ...
                <p className="text-sm text-slate-500">Try refreshing the page or check your API connection.</p>
              </div>
            )}
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-500/5">
      <div className="flex items-start justify-between">
        <div className={`rounded-xl border p-2.5 transition-colors ${colors[color]}`}>
      ...
      <div className="mt-4">
        ...
        <p className="mt-1 text-xs text-slate-400 line-clamp-1">{description}</p>
      </div>
