...
import { ArrowLeft, MapPin, Monitor, Package, Info, Calendar } from 'lucide-react';
...
export default function StoreDetailClient({ 
  ...
  const storeData = useMemo(() => {
    return data.filter(item => item.storeLocation === storeName);
  }, [data, storeName]);

  const stats = useMemo(() => {
    const totalLifts = storeData.reduce((acc, curr) => acc + parseInt(curr.Total || '0'), 0);
    const uniqueScreens = new Set(storeData.map(item => item.screenId)).size;
    const uniqueItems = new Set(storeData.map(item => item.libraryItemId)).size;
  ...
  if (storeData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-slate-500 mt-2">We couldn't find any data for "{storeName}"</p>
        <Link href="/dashboard" className="mt-6 text-blue-600 font-bold hover:underline flex items-center gap-2">
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          <Link href="/dashboard" className="text-sm font-bold text-blue-600 flex items-center gap-1.5 hover:gap-2 transition-all">
          ...
          <div className="flex items-center gap-3">
             ...
             <div>
                <h1 className="text-3xl font-black text-slate-800 tracking-tight">{storeName}</h1>
                <p className="text-slate-500 flex items-center gap-1.5 text-sm">
                  <Monitor className="h-3.5 w-3.5" />
                  Monitoring {stats.uniqueScreens} active kiosks in this branch
                </p>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm">
           ...
           <span className="text-sm font-medium text-slate-500 underline decoration-blue-500 underline-offset-4">Last 10 minutes</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Total Interactions</p>
          <div className="flex items-baseline gap-2 mt-2">
            <h4 className="text-4xl font-black text-slate-800">{stats.totalLifts.toLocaleString()}</h4>
            <span className="text-xs font-bold text-green-600 bg-green-50 px-1.5 py-0.5 rounded">LIFTS</span>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-indigo-700 bg-indigo-50/30 border-indigo-100">
          <p className="text-indigo-500 text-xs font-bold uppercase tracking-widest">Engaged Content</p>
          <div className="flex items-baseline gap-2 mt-2">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-blue-700 bg-blue-50/30 border-blue-100">
          <p className="text-blue-500 text-xs font-bold uppercase tracking-widest">Store Section Health</p>
          <div className="flex items-baseline gap-2 mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500">Distribution of lifts per item</p>
          </div>
        ...
        <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-xl shadow-slate-200">
          ...
          <div className="space-y-4">
             {Array.from(new Set(storeData.map(s => s.screenLabel))).map((screen, idx) => (
          ...
          <div className="mt-8 pt-8 border-t border-white/10 text-center">
             <p className="text-white/40 text-[10px] font-bold uppercase tracking-[0.2em]">End of Live Data Stream</p>
          </div>
        </div>
      </div>
    </div>
